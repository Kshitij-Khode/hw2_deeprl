...
class QNetwork():
    ...
    def get_qvals(self, state):
        return self.model.predict(np.array([[state[0], state[1], state[2], state[3]]]))[0]
...
class DQN_Agent():
    ...
    def train(self):
        ...
        for ep in xrange(max_ep):
            ...
            for _ in xrange(max_ep_len):
                ...
                qchoice = random = np.random.uniform()
                state_batch, q_batch = [], []
    ...
    def test(self, model_file=None):
        ...
        for _ in xrange(max_ep):
            ...
            for _ in xrange(max_ep_len):
                action = self.epsilon_greedy_policy(self.q_net1.get_qvals(nstate), self.min_eps)
