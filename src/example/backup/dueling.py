...
class QNetwork():
    ...
    def __init__(self, env_name):
        ...
        self.model.add(Dense(self.nact, input_shape=(2,)))
...
class DQN_Agent():
    ...
    def epsilon_greedy_policy(self, q_values, eps):
        ...
        if np.random.uniform() < eps: return np.random.random_integers(0, self.q_net1.nact-1)
        else: return self.greedy_policy(q_values)

    def random_policy(self):
        return np.random.random_integers(0, self.q_net1.nact-1)
    ...
    def train(self, render=False, verb=0):
        ...
        for ep in xrange(max_ep):
            nstate = env.reset()
            print("q vals at initial state: %s, q1lrate: %s, q2lrate: %s" % (self.q_net1.get_qvals(nstate)+self.q_net2.get_qvals(nstate), self.q_net1.lrate, self.q_net2.lrate))
            ...
            for _ in xrange(max_epi_len):
                ...
                for pstate, action, rew, nstate, term in self.rep_mem.sample_batch():
                    if random < 0.5:
                        y_n = self.q_net1.get_qvals(pstate)
                        y_n[action] = rew if term else rew+gamma*np.max(self.q_net1.get_qvals(nstate))
                    else:
                        y_n = self.q_net2.get_qvals(pstate)
                        y_n[action] = rew if term else rew+gamma*np.max(self.q_net2.get_qvals(nstate))
            ...
            if ars[-1] <= -110:
                ...
                if len(ds) > 0 and all(d != 0 and abs(d) < 30 for d in ds):
                    self.q_net1.lrate -= self.q_net1.dlrate
                    self.q_net2.lrate -= self.q_net2.dlrate
                    if verb > 0: print('DQN_Agent::train::lrate(%s)' % self.q_net1.lrate)
            else: break
    ...
    def test(self, model_file=None, render=False, verb=0):
        ...
        for _ in xrange(max_ep):
            ...
            for _ in xrange(max_epi_len):
                ...
                nstate, reward, term, info = env.step(self.epsilon_greedy_policy(self.q_net1.get_qvals(nstate)+self.q_net2.get_qvals(nstate), 0.05))
